...
export default function PurchasePage() {
  ...
  const load = async () => {
    setLoading(true);
    const [p, v] = await Promise.all([getAll("purchases"), getAll("vendors")]);
    setPurchases(p); setVendors(v); setLoading(false);
  };
  useEffect(() => { load(); }, []);
  ...
  // When vendor selected in PO header
  ...
  // Products filtered by selected vendor
  const vendorProducts = draftPO?.vendorName
    ? vendors.filter(v=>v.vendorName===draftPO.vendorName)
    : [];
    ...
    <div>
      ...
      {tab==="Add Purchase"&&(
        <div style={{display:"flex",flexDirection:"column",gap:20}}>
          ...
          :<>
            {/* PO Header — PO Number, Date, Vendor */}
                  ...
                  <div key={row._id} className="ims-panel" style={{position:"relative"}}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr 1fr",gap:12,alignItems:"end"}}>
                      ...
                      <div style={{display:"flex",flexDirection:"column",gap:5}}>
                        ...
                        <input type="number" className="ims-input" value={row.costPerHead} onChange={e=>updateRow(row._id,"costPerHead",e.target.value)} placeholder="Auto"/>
                      </div>
                      <div style={{display:"flex",flexDirection:"column",gap:5}}>
